...
<div class="mtop15 preview-top-wrapper">
    ...
    if ($identity_confirmation_enabled == '1' && $can_be_confirmed) {
        // Load the core identity confirmation modal (signature UI + JS hooks),
        // but we will strip the email field entirely (DOM + validation rules)
        // because delivery note acceptance should not require/collect email.
        get_template_part('identity_confirmation_form', ['formAction' => site_url('delivery_notes/client/sign_delivery/' . $delivery_note->id)]);
    }
    ?>

    <?php if ($identity_confirmation_enabled == '1' && $can_be_confirmed) { ?>
    <script>
    // Remove email field from the identity confirmation modal (and its validation),
    // without editing the core template part.
    (function($){
        'use strict';

        function stripEmailField($modal){
            if (!$modal || !$modal.length) { return; }

            // Remove any email inputs (visible or hidden)
            var $email = $modal.find('input[type="email"], input[name="acceptance_email"], #acceptance_email');
            $email.each(function(){
                var $el = $(this);
            ...
            $modal.find('input[type="hidden"][name="acceptance_email"]').remove();

            // Remove validator rules/messages if already attached
            var $form = $modal.find('form').first();
            if ($form.length) {
                var v = $form.data('validator');
                if (v && v.settings) {
                    if (v.settings.rules && v.settings.rules.acceptance_email) {
                        delete v.settings.rules.acceptance_email;
                    }
                    if (v.settings.messages && v.settings.messages.acceptance_email) {
                        delete v.settings.messages.acceptance_email;
                    }
                }
                // If rules were added via .rules('add')
                try {
                    $form.find('[name="acceptance_email"]').rules('remove');
                } catch (e) {}

                // Ensure the field is gone from the form submit payload
                $form.find('[name="acceptance_email"]').remove();
            }
        }

        $(function(){
            var $modal = $('#identityConfirmationModal');
            if (!$modal.length) { return; }

            // Run immediately
            stripEmailField($modal);

            // Run when modal is shown (some scripts attach validation on show)
            $modal.on('shown.bs.modal', function(){
                stripEmailField($modal);
            });

            // Run shortly after (covers late init)
            setTimeout(function(){ stripEmailField($modal); }, 250);
        });
    })(jQuery);
    </script>
    <?php } ?>
