            ...
            <div class="col-md-6">
                <div class="tw-ml-3">
                    ...
                    <div class="row">
						...
						<!-- Recurring UI (ΑΝΤΙ render_select) -->
						<div class="col-md-12">
						  <?php
							$dn = isset($delivery_note) ? $delivery_note : null;
							$dn_recurring        = (int)($dn->recurring ?? 0);
							$dn_recurring_type   =        ($dn->recurring_type ?? 'month');
							$dn_cycles           = (int)($dn->cycles ?? 0);
							$dn_custom_recurring = (int)($dn->custom_recurring ?? 0);

							// Τι θα δείξει το dropdown:
							// - Νέο DN -> '0' (No)
							// - Υπάρχον DN: αν έχει custom -> 'custom', αλλιώς ο αριθμός των μηνών
							$chooser_value = '0';
							if (isset($delivery_note)) {
							  if ($dn_custom_recurring)       { $chooser_value = 'custom'; }
							  elseif ($dn_recurring > 0)      { $chooser_value = (string)$dn_recurring; }
							  else                            { $chooser_value = '0'; }
							}
						  ?>

						  <div class="form-group mtop10">
							<label for="recurring_chooser"><?php echo _l('dn_recurring_invoice_q'); ?></label>
							<select id="recurring_chooser" name="recurring_chooser"
									class="selectpicker" data-width="100%">
							  <!-- ΧΩΡΙΣ κενή επιλογή -->
							  <option value="0" <?php echo ($chooser_value==='0' ? 'selected' : ''); ?>>
								<?php echo _l('dn_recurring_no'); ?>
							  </option>
							  <?php for ($i=1; $i<=12; $i++): ?>
								<option value="<?php echo $i; ?>" <?php echo ($chooser_value===(string)$i ? 'selected' : ''); ?>>
								  <?php echo _l('dn_every_x_months', $i); ?>
								</option>
							  <?php endfor; ?>
							  <option value="custom" <?php echo ($chooser_value==='custom' ? 'selected' : ''); ?>>
								<?php echo _l('dn_recurring_custom'); ?>
							  </option>
							</select>
						  </div>

						  <?php echo form_hidden('custom_recurring', $dn_custom_recurring); ?>

						  <!-- Custom interval -->
						  <div id="recurring_custom_wrap" class="<?php echo ($chooser_value==='custom' ? '' : 'hide'); ?>">
							<div class="row">
							  <div class="col-md-6">
								<?php echo render_input(
								  'recurring',
								  _l('dn_recurring_repeats'),
								  ($chooser_value==='custom' ? max(1,$dn_recurring) : 1),
								  'number',
								  ['min'=>1,'step'=>1]
								); ?>
							  </div>
							  <div class="col-md-6">
								<label for="recurring_type"><?php echo _l('dn_recurring_type'); ?></label>
								<select id="recurring_type" name="recurring_type" class="selectpicker" data-width="100%">
								  <option value="day"   <?php echo ($dn_recurring_type==='day'   ? 'selected' : ''); ?>><?php echo _l('dn_day_s'); ?></option>
								  <option value="week"  <?php echo ($dn_recurring_type==='week'  ? 'selected' : ''); ?>><?php echo _l('dn_week_s'); ?></option>
								  <option value="month" <?php echo ($dn_recurring_type==='month' ? 'selected' : ''); ?>><?php echo _l('dn_month_s'); ?></option>
								  <option value="year"  <?php echo ($dn_recurring_type==='year'  ? 'selected' : ''); ?>><?php echo _l('dn_year_s'); ?></option>
								</select>
							  </div>
							</div>
						  </div>
						  ...
						  <!-- Cycles -->
						  <div id="recurring_cycles_wrap" class="<?php echo ($chooser_value==='0' ? 'hide' : ''); ?>">
							<div class="row">
							  <div class="col-md-12">
								<div class="form-group recurring-cycles">
								  <label for="cycles_input"><?php echo _l('dn_total_cycles'); ?></label>
								  <div class="input-group">
									<!-- ΥΠΟΒΑΛΛΕΤΑΙ ΠΑΝΤΑ -->
									<input type="hidden" name="cycles" id="cycles_hidden" value="<?php echo (int)$dn_cycles; ?>">
									<!-- Ορατό, χωρίς name -->
									<input type="number" class="form-control" id="cycles_input" value="<?php echo (int)$dn_cycles; ?>">
									<div class="input-group-addon">
									  <div class="checkbox">
										<?php $infinity = ($chooser_value==='0' || $dn_cycles===0); ?>
										<input type="checkbox" id="cycles_infinity" <?php echo $infinity ? 'checked' : ''; ?>>
										<label for="cycles_infinity"><?php echo _l('dn_infinity'); ?></label>
									  </div>
									</div>
								  </div>
								  <small class="text-muted"><?php echo _l('dn_cycles_help'); ?></small>
								</div>
							  </div>
							</div>
						  </div>
