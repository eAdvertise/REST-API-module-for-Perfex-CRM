...
[English](README.md) · [Ελληνικά](README.el.md) · [עברית](README.he.md) · 🌐 **简体中文** · [Español](README.es.md) · [Português (BR)](README.pt-BR.md) · [Italiano](README.it.md) · [Français](README.fr.md) · [Deutsch](README.de.md) · [Türkçe](README.tr.md) · [Tiếng Việt](README.vi.md) · [ไทย](README.th.md) · [العربية](README.ar.md)

> 面向 [REST API module for eAD-CRM](https://www.eadvertise.eu/) 的
> 开箱即用 **Postman 集合**、**代码片段**（cURL、PHP、Python、JavaScript）以及资源
> **目录** —
> 是 **将 eAD-CRM 与 AI 智能体及第三方应用连接** 的最快方式。
...
**eAD-CRM REST API** 让你能够通过简洁的 HTTP/JSON 接口读写客户、潜在客户、发票、报价、项目、
...
模块（由 **eAdvertise** 出品）的实用配套资料：可复制粘贴的示例、可导入的 Postman 集合，以及完整的
端点目录。

- 🧩 **获取模块：** https://www.eadvertise.eu/
- 📖 **API 指南 / 在线文档：** https://apidoc.eadcrm.eu/
...
对 eAD-CRM REST API 的每个请求都通过 **`Authtoken`** 请求头进行认证。请在 eAD-CRM 管理后台的
**API → API Management** 中创建令牌（需先激活
[REST API 模块](https://www.eadvertise.eu/)），
...
1. 打开 Postman → **Import** → 拖入 [`postman/eadcrm-rest-api.postman_collection.json`](postman/eadcrm-rest-api.postman_collection.json)。
2. 导入环境 [`postman/eadcrm-rest-api.postman_environment.json`](postman/eadcrm-rest-api.postman_environment.json)。
...
> 每种资源的确切请求字段都记录在官方
> **[API 指南](https://apidoc.eadcrm.eu/)** 中。此处的片段覆盖了最常见的流程。
...
eAD-CRM REST API 常用于 **将 eAD-CRM 与 AI 智能体及第三方应用连接**：

- **AI 助手（MCP）** — 让 Claude、ChatGPT 或 Cursor 通过 `/api/mcp` 读取并更新你的 CRM。
- **Zapier / Make / n8n** — 通过开箱即用的轮询触发器（`/api/zapier/*`）实现无代码自动化。
- **Webhooks** — 将 eAD-CRM 事件（新发票、新潜在客户，共 124 个事件）推送到 Slack、Discord 或你自己的后端，并使用 HMAC 签名。
- **Google Sheets / Power Automate** — 将客户、发票或付款同步到电子表格和仪表盘。
- **自定义应用与门户** — 基于你的 eAD-CRM 数据构建移动应用或客户门户。
- **会计与电商** — 将发票和商品项与外部计费或商店平台同步。

以上全部由
[REST API for eAD-CRM](https://www.eadvertise.eu/) 模块提供支持。
...
**eAD-CRM 有 Postman 集合吗？**
有 — 导入 [`postman/eadcrm-rest-api.postman_collection.json`](postman/eadcrm-rest-api.postman_collection.json)
及随附的环境，设置你的 `base_url` 和 `authtoken`，即可开始发送请求。

**我该如何通过 eAD-CRM API 创建发票？**
...
> 由 [eAdvertise](https://www.eadvertise.eu/) 出品。

- 🛒 **购买 / 了解更多：** https://www.eadvertise.eu/
- 📖 **文档：** https://apidoc.eadcrm.eu/
- 💬 **支持：** https://www.eadvertise.eu/

欢迎贡献更多示例 — 请参见 [`CONTRIBUTING.md`](CONTRIBUTING.md)。

## 许可证

本仓库中的示例代码基于 [MIT License](LICENSE) 发布。"eAD-CRM" 是其各自所有者的商标；
REST API 模块是 eAdvertise 的商业产品。
