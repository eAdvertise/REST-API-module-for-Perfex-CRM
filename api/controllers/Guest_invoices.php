...
class Guest_invoices extends REST_Controller
{
    ...
    public function checkout_post()
    {
        ...
        $payment_id = $this->record_payment_safely($payment_data, $requestId);
        if (!$payment_id) {
    ...
    private function record_payment_safely(array $payment_data, string $requestId): int
    {
        $this->ensure_app_hooks();

        try {
            return (int)$this->payments_model->add($payment_data);
        } catch (Throwable $e) {
            log_message('error', '[API Guest Checkout] Payments_model->add failed. request_id=' . $requestId . ' error=' . $e->getMessage());

            $existingPaymentId = $this->find_existing_payment_record($payment_data);
            if ($existingPaymentId > 0) {
                return $existingPaymentId;
            }

            return $this->insert_payment_record_directly($payment_data, $requestId);
        }
    }

    private function find_existing_payment_record(array $payment_data): int
    {
        $this->db->where('invoiceid', (int)$payment_data['invoiceid']);
        $this->db->where('amount', (float)$payment_data['amount']);
        $this->db->where('paymentmode', (int)$payment_data['paymentmode']);

        if (!empty($payment_data['transactionid'])) {
            $this->db->where('transactionid', (string)$payment_data['transactionid']);
        } else {
            $this->db->where('date', (string)$payment_data['date']);
            $this->db->where('note', (string)$payment_data['note']);
        }
        ...
        return $payment ? (int)$payment->id : 0;
    }

    private function insert_payment_record_directly(array $payment_data, string $requestId): int
    {
        $table = db_prefix() . 'invoicepaymentrecords';
        $insert = [
            'invoiceid'     => (int)$payment_data['invoiceid'],
            'amount'        => (float)$payment_data['amount'],
            'paymentmode'   => (int)$payment_data['paymentmode'],
            'date'          => (string)$payment_data['date'],
            'daterecorded'  => date('Y-m-d H:i:s'),
            'transactionid' => (string)$payment_data['transactionid'],
            'note'          => (string)$payment_data['note'],
        ];
        ...
        if ($paymentId > 0) {
            if (function_exists('update_invoice_status')) {
                try {
                    update_invoice_status((int)$payment_data['invoiceid'], true);
                } catch (Throwable $e) {
                    log_message('error', '[API Guest Checkout] Fallback payment recorded but invoice status update failed. request_id=' . $requestId . ' error=' . $e->getMessage());
                }
            }

            log_message('error', '[API Guest Checkout] Payment recorded through direct fallback. request_id=' . $requestId . ' payment_id=' . $paymentId);
        }
