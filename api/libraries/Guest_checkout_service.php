...
class Guest_checkout_service
{
    ...
    public function findOrCreateGuest(array $payload, array $options = [])
    {
        ...
        if ($contact) {
            ...
            if ($client_id <= 0 || !$this->clientExists($client_id)) {
                $client_id = $this->createClientForExistingContact($payload, $contact, $name, $email);
                if ($client_id <= 0) {
                    return [0, $contact_id, 'Existing contact found but failed to repair missing client'];
                }

                log_message('error', '[API Guest Checkout] Re-linked existing contact to repaired client. contact_id=' . $contact_id . ' client_id=' . $client_id . ' email=' . $email);
            }
    ...
    private function clientExists($client_id)
    {
        if ((int)$client_id <= 0) {
            return false;
        }
        ...
        return $this->CI->db
            ...
            ->count_all_results(db_prefix() . 'clients') > 0;
    }

    private function createClientForExistingContact(array $payload, $contact, $name, $email)
    {
        $company = trim((string)($payload['company'] ?? $payload['company_name'] ?? ''));
        if ($company === '' && $name !== '') {
            $company = $name;
        ...
        if ($company === '') {
            $company = trim((string)($contact->firstname ?? '') . ' ' . (string)($contact->lastname ?? ''));
        }
        ...
        $client_id = (int)$this->CI->clients_model->add($this->buildClientDataFromPayload($payload, $company));
        if ($client_id <= 0) {
            return 0;
        }

        $this->CI->db->where('id', (int)$contact->id)->update(db_prefix() . 'contacts', [
            'userid'     => $client_id,
            'is_primary' => 1,
        ]);
