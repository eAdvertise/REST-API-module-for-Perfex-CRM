    ...
    public function data_put($id = "") {
        ...
        if (empty($id) && !is_numeric($id)) {
            ...
            if (is_object($is_exist)) {
                ...
                if ($output > 0 && !empty($output)) {
                    $file_provided = isset($_FILES['file']['name']) && !empty($_FILES['file']['name']);
                    // Only replace existing attachments when a new file is actually
                    // uploaded, so a field-only PUT does not wipe them.
                    if ($file_provided) {
                        $this->expenses_model->delete_expense_attachment($output);
                    }
                    $saved_attachments = $file_provided ? (int) $this->handle_expense_attachments_array($output) : 0;
                    $message = array('status' => TRUE, 'message' => "Expense Updated Successfully",);
                    if ($file_provided) {
                        $message['attachments_saved'] = $saved_attachments;
                        if ($saved_attachments === 0) {
                            $message['message'] = 'Expense updated, but the attachment could not be saved.';
                        }
                    }
                    $this->response($message, REST_Controller::HTTP_OK);
                } else {
        ...
        if (isset($_FILES[$index_name]['name']) && ($_FILES[$index_name]['name'] != '' || is_array($_FILES[$index_name]['name']) && count($_FILES[$index_name]['name']) > 0)) {
            ...
            for ($i = 0; $i < count($_FILES[$index_name]['name']); $i++) {
                ...
                if (!empty($tmpFilePath) && $tmpFilePath != '') {
                    ...
                    // move_uploaded_file() only accepts genuine HTTP POST uploads
                    // (is_uploaded_file). Files parsed from a raw PUT body by
                    // Parse_Input_Stream are written to a temp path, so fall back to
                    // rename() for those.
