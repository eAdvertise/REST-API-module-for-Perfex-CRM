...
class Parse_Input_Stream
{
    ...
    private function file_stream($data)
    {
		...
		$idx = strpos( $data, "\r\n\r\n" );
		if ( $idx === FALSE ) {
			log_message('error', 'ParseInputStream.file_stream(): Could not locate header separator in data.');
		} else {
			...
			while ( $header !== FALSE ) {
				if ( substr($header, 0, strlen("Content-Disposition: ")) == "Content-Disposition: " ) {
					...
					log_message('debug', 'PARSEINPUTSTREAM: Skipping Header: ' . $header);
				}
	...
	function parse_parameter( &$params, $parameter, $value ) {
		if ( strpos($parameter, '[') !== FALSE ) {
			...
			if ( preg_match( '/^([^[]*)\[([^]]*)\](.*)$/', $parameter, $match ) ) {
				...
				log_message('error', "ParseInputStream.parse_parameter() Parameter name regex failed: '" . $parameter . "'");
			}
		} else {
