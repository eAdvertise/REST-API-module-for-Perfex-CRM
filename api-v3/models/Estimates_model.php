...
class Estimates_model extends App_Model {
    ...
    public function get($id = '', $where = [], $playground = false) {
        ...
        if (is_numeric($id)) {
            ...
            if ($estimate) {
                ...
                $estimate->items = $this->invoice_items_model->get_items_by_type('estimate', $id, $playground);
    ...
    public function clear_signature($id, $playground = false) {
        ...
        if ($estimate) {
            ...
            if (!empty($estimate->signature)) {
                unlink($this->misc_model->get_upload_path_by_type('estimate', $playground) . $id . '/' . $estimate->signature);
            }
            return true;
        }
        return false;
    ...
    public function convert_to_invoice($id, $client = false, $draft_invoice = false, $playground = false) {
        ...
        $this->load->model('invoices_model');
        $id = $this->invoices_model->add($new_invoice_data, $playground);
        if ($id) {
                ...
                foreach ($cfEstimates as $field) {
                    ...
                    if (isset($tmpSlug[1])) {
                        ...
                        if (count($cfTransfer) == 1 && ((similarity($field['name'], $cfTransfer[0]['name']) * 100) >= CUSTOM_FIELD_TRANSFER_SIMILARITY)) {
                            $value = $this->custom_fields_model->get_custom_field_value($_estimate->id, $field['id'], 'estimate', false, $playground);
                            if ($value == '') {
                                continue;
                            }
    ...
    public function copy($id, $playground = false) {
        ...
        foreach ($_estimate->items as $item) {
            ...
            foreach ($custom_fields_items as $cf) {
                $new_estimate_data['newitems'][$key]['custom_fields']['items'][$cf['id']] = $this->custom_fields_model->get_custom_field_value($item['id'], $cf['id'], 'items', false, $playground);
                if (!defined('COPY_CUSTOM_FIELDS_LIKE_HANDLE_POST')) {
                    define('COPY_CUSTOM_FIELDS_LIKE_HANDLE_POST', true);
                }
            }
        ...
        if ($id) {
            $this->load->model('custom_fields_model');
            $custom_fields = $this->custom_fields_model->get_custom_fields('estimate', [], false, $playground);
            ...
            $this->load->model('misc_model');
            $this->misc_model->handle_tags_save($tags, $id, 'estimate');
    ...
    public function add($data) {
        ...
        if ($insert_id) {
            ...
            if ($estimateRequestID !== false && $estimateRequestID != '') {
                ...
                $this->estimate_request_model->update_request_status(['requestid' => $estimateRequestID, 'status' => $completedStatus->id, ]);
            }
            if (isset($custom_fields)) {
                handle_custom_fields_post($insert_id, $custom_fields);
            }
            $this->load->model('misc_model');
            $this->misc_model->handle_tags_save($tags, $insert_id, 'estimate');
            $this->load->model('invoice_items_model');
            foreach ($items as $key => $item) {
                if ($itemid = $this->invoice_items_model->add_new_sales_item_post($item, $insert_id, 'estimate', $playground)) {
                    $this->invoice_items_model->_maybe_insert_post_item_tax($itemid, $item, $insert_id, 'estimate', $playground);
                }
            }
    ...
    public function update($data, $id) {
        ...
        $this->load->model('misc_model');
        if (isset($data['tags'])) {
            if ($this->misc_model->handle_tags_save($data['tags'], $id, 'estimate')) {
                $affectedRows++;
            }
        }
        $data['billing_street'] = trim($data['billing_street']);
        ...
        foreach ($items as $key => $item) {
            ...
            if (!isset($item['taxname']) || (isset($item['taxname']) && count($item['taxname']) == 0)) {
                ...
                if ($this->invoice_items_model->_maybe_insert_post_item_tax($item['itemid'], $item, $id, 'estimate', $playground)) {
                    $affectedRows++;
                }
            }
        }
        foreach ($newitems as $key => $item) {
            if ($new_item_added = $this->invoice_items_model->add_new_sales_item_post($item, $id, 'estimate', $playground)) {
                $this->invoice_items_model->_maybe_insert_post_item_tax($new_item_added, $item, $id, 'estimate', $playground);
    ...
    public function mark_action_status($action, $id, $client = false, $playground = false) {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            if ($client == true) {
                ...
                $invoiceid = false;
                $invoiced = false;
                $contact_id = !is_client_logged_in() ? $this->clients_model->get_primary_contact_user_id($estimate->clientid, $playground) : get_contact_user_id();
                if ($action == 4) {
     ...
     * @return mixed
     */
    public function get_attachments($estimate_id, $id = '', $playground = false) {
    ...
    public function delete_attachment($id, $playground = false) {
        ...
        if ($attachment) {
            if (empty($attachment->external)) {
                unlink($this->misc_model->get_upload_path_by_type('estimate', $playground) . $attachment->rel_id . '/' . $attachment->file_name);
            }
            ...
            if (is_dir($this->misc_model->get_upload_path_by_type('estimate', $playground) . $attachment->rel_id)) {
                // Check if no attachments left, so we can delete the folder also
                $other_attachments = list_files($this->misc_model->get_upload_path_by_type('estimate', $playground) . $attachment->rel_id);
                if (count($other_attachments) == 0) {
                    // okey only index.html so we can delete the folder also
                    delete_dir($this->misc_model->get_upload_path_by_type('estimate', $playground) . $attachment->rel_id);
                }
            }
        }
        return $deleted;
    ...
    public function delete($id, $simpleDelete = false) {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            $this->load->model('misc_model');
            $this->misc_model->delete_tracked_emails($id, 'estimate', $playground);
    ...
    public function send_expiry_reminder($id, $playground = false) {
        ...
        $this->load->model('clients_model');
        $contacts = $this->clients_model->get_contacts($estimate->clientid, ['active' => 1, 'estimate_emails' => 1], $playground);
        foreach ($contacts as $contact) {
