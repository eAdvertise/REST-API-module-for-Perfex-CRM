...
class Customers extends REST_Controller {
    ...
    private function normalize_customer_custom_fields($custom_fields) {
        ...
        if (isset($custom_fields['customer']) && is_array($custom_fields['customer'])) {
            $custom_fields['customers'] = $custom_fields['customer'];
            unset($custom_fields['customer']);
            return $custom_fields;
        }
        ...
        if ($looks_flat) {
            return ['customers' => $custom_fields];
        }
    ...
    public function data_get($id = '') {
        // If the id parameter doesn't exist return all customers with pagination
        ...
        if ($data) {
            ...
            // Apply pagination if retrieving all customers (no specific id)
    ...
    public function data_search_get($key = '') {
        ...
        // This allows: /api/customers/search/term OR /api/customers/search?q=term
        ...
        if (empty($key)) {
            $this->response(['status' => FALSE, 'message' => 'Search term is required. Use /api/customers/search/{term} or /api/customers/search?q={term}'], REST_Controller::HTTP_BAD_REQUEST);
            return;
        }
    ...
    public function data_put($id = '') {
        ...
        $this->load->model('clients_model');
        $existing = $this->clients_model->get($id);
        if (!$existing) {
            $message = array('status' => FALSE, 'message' => 'Customer not found');
            $this->response($message, REST_Controller::HTTP_NOT_FOUND);
            return;
        }
        ...
        $update_data = $this->api_sanitize_update_payload('clients', $update_data, ['custom_fields', 'groups_in']);

        $custom_fields = $this->input->post('custom_fields', TRUE);
        if (!empty($custom_fields)) {
            $custom_fields = $this->normalize_customer_custom_fields($custom_fields);
            $update_data['custom_fields'] = $custom_fields;
        }

        $core_data = $update_data;
        ...
        // Check if update was successful or if no changes were made (both are valid)
        // The model returns TRUE/ID on success, FALSE on actual failure
        if ($output !== FALSE) {
            // success (including when no changes were made)
            $message = array('status' => TRUE, 'message' => 'Customers Update Successful.');
            $this->response($message, REST_Controller::HTTP_OK);
        } else {
            // error
