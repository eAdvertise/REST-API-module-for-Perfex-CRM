...
class Webhooks extends AdminController
{
    public function __construct()
    {
        ...
        if (!$this->app_modules->is_active('webhooks')) {
            access_denied();
        }
    }
        ...
        if ($this->input->post()) {
            ...
            if ($is_valid_url) {
                $data = [
                    ...
                    'webhook_after_number' => !empty($posted_data['webhook_after_number']) ? $posted_data['webhook_after_number'] : null,
                    'webhook_after_type' => !empty($posted_data['webhook_after_type']) ? $posted_data['webhook_after_type'] : null
                ];
                if (is_numeric($id)) {
    ...
    public function webhooks_cron_job_queue_table()
    {
        if (!$this->input->is_ajax_request()) {
            ajax_access_denied();
        }

        $this->app->get_table_data(module_views_path(WEBHOOKS_MODULE, 'tables/webhooks_cron_job_queue_table'));
    }

    public function delete_scheduled_call($id): void
    {
        if ($this->webhooks_model->delete_scheduled_call(['id' => $id])) {
            set_alert('success', _l('deleted', 'webhook'));
        } else {
            set_alert('danger', _l('problem_deleting', 'webhook'));
        }
    ...
    public function test_webhook()
    {
        ...
        if (!$is_valid_url) {
            echo json_encode(['success'=> 'danger', 'message' => _l('webhook_test_invalid_url')]);
        }

        $request_method = strtoupper($posted_data['request_method'] ?? 'GET');
        $request_format = strtoupper($posted_data['request_format'] ?? 'FORM');

        if (in_array($request_method, ['GET', 'DELETE'], true) && 'JSON' === $request_format) {
            echo json_encode(['success'=> 'danger', 'message' => _l('webhook_test_json_not_supported')]);
        }

        $headers = [];
        $request_headers = $posted_data['header'] ?? [];
        if (is_array($request_headers)) {
            foreach ($request_headers as $header) {
                $header_choice = trim((string) ($header['header_choice'] ?? ''));
                $header_custom_choice = trim((string) ($header['header_custom_choice'] ?? ''));
                $header_value = trim((string) ($header['value'] ?? ''));

                if (strtolower($header_choice) === 'custom') {
                    $header_choice = $header_custom_choice;
                }
                ...
                $headers[$header_choice] = $header_value;
            }
        }

        $custom_body = [];
        $request_body = $posted_data['body'] ?? [];
        if (is_array($request_body)) {
            foreach ($request_body as $body_row) {
                $key = trim((string) ($body_row['key'] ?? ''));
                $value = trim((string) ($body_row['value'] ?? ''));
        ...
        $test_payload = array_merge([
            'is_test_webhook' => true,
            'message' => 'This is a test webhook request.',
            'webhook_name' => $posted_data['webhook_name'] ?? '',
            'webhook_for' => $posted_data['webhook_for'] ?? '',
            'requested_at' => date('c'),
        ], $custom_body);
        ...
        if ('JSON' === $request_format) {
            ...
            $request_data = json_encode($test_payload);
        }

        $add_test_failure_log = function ($response_code, $response_data) use ($posted_data, $is_valid_url, $request_method, $request_format, $headers, $request_data) {
            $insert_data = [
                'webhook_action_name' => !empty($posted_data['webhook_name']) ? $posted_data['webhook_name'] : 'Test Webhook',
                'request_url'    => $is_valid_url,
                'request_method' => $request_method,
                'request_format' => $request_format,
                'webhook_for'    => $posted_data['webhook_for'] ?? '',
                'webhook_action' => json_encode(['test']),
                'request_header' => json_encode($headers),
                'request_body'   => is_array($request_data) ? json_encode($request_data) : $request_data,
                'response_code'  => $response_code,
                'response_data'  => $response_data,
            ];

            $this->webhooks_model->add_log($insert_data);
        };
        ...
        try {
            ...
            $response_preview = trim(strip_tags((string) $request->body));
            $response_preview = substr($response_preview, 0, 500);
            $is_success = $response_code >= 200 && $response_code < 300;

            if (!$is_success) {
                $add_test_failure_log($response_code, htmlentities((string) $request->body));
            }

            echo json_encode(['success'=>$is_success ? 'success' : 'danger', 'message' => $is_success ? _l('webhook_test_success'): _l('webhook_test_failed')]);
            
        } catch (\Exception $e) {
            $add_test_failure_log('EXCEPTION', $e->getMessage());

            echo json_encode(['success'=> 'danger', 'message' => _l('webhook_test_exception')]);
        }
    }
}
